...
You can use the functions "mutate()" and "factor()", defining then the levels (0,1) and the labels ("Yes" and "No").
...
And if you use the function "cdplot()" to plot the changes in casecont with different values of the other variables?

```{r}
cdplot(CC_noccardia$casecont ~ CC_noccardia$dcpct)
cdplot(CC_noccardia$casecont ~ CC_noccardia$dneo )
cdplot(CC_noccardia$casecont ~ CC_noccardia$dclox )
```
...
After having the model, calculate the Confidence Intervals (CI) of the estimates, using the function "confint.default("model")".
...
Can you now calculate the probabability of mastitis in case animals are treated with neomicyn?
...
- If you transform it in odds ratio, using the functions exp(coef("model")) and "exp(confint.default("model")))", what does it mean?
...
```{r}
#for 30%
exp(coef(mod2)[1]+coef(mod2)[2]*30) / (1+ (exp(coef(mod2)[1]+coef(mod2)[2]*30)))

#for 50%
exp(coef(mod2)[1]+coef(mod2)[2]*50) / (1+ (exp(coef(mod2)[1]+coef(mod2)[2]*50)))
